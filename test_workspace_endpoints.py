...
async def test_workspace_flow():
    ...
    async with httpx.AsyncClient() as client:
        ...
        res = await client.post(f"{BASE_URL}/workspaces/", json=create_payload, headers=headers_owner)
        ...
        res = await client.get(f"{BASE_URL}/workspaces/{new_ws_id}/members", headers=headers_owner)
        assert res.status_code == 200, f"Failed: {res.status_code} {res.text}"
        members = res.json()
        ...
        res = await client.post(f"{BASE_URL}/workspaces/{new_ws_id}/members", json=invite_payload, headers=headers_owner)
        assert res.status_code == 201, f"Failed: {res.status_code} {res.text}"
        ...
        res = await client.patch(f"{BASE_URL}/workspaces/{new_ws_id}/members/{MEMBER_ID}", json=role_payload, headers=headers_owner)
        ...
        res = await client.delete(f"{BASE_URL}/workspaces/{new_ws_id}", headers=headers_member)
        assert res.status_code == 403, f"Unexpected: {res.status_code} {res.text}"
        print(f"Forbidden check passed. Status code: {res.status_code}, Detail: {res.json().get('detail')}")
        ...
        res = await client.patch(f"{BASE_URL}/workspaces/{new_ws_id}", json=update_payload, headers=headers_owner)
        assert res.status_code == 200, f"Failed: {res.status_code} {res.text}"
        updated_ws = res.json()
        print(f"Workspace updated: name='{updated_ws['name']}', slug='{updated_ws['slug']}', desc='{updated_ws['description']}'")
        assert updated_ws["name"] == f"{ws_name} Updated"

        print("\n--- 10. Owner deletes workspace (Should succeed with 204) ---")
        res = await client.delete(f"{BASE_URL}/workspaces/{new_ws_id}", headers=headers_owner)
        assert res.status_code == 204, f"Failed: {res.status_code} {res.text}"
        print("Workspace deleted successfully.")

        print("\n--- 11. Verify workspace no longer accessible ---")
        res = await client.get(f"{BASE_URL}/workspaces/{new_ws_id}", headers=headers_owner)
