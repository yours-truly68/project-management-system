...
export function CreateColumnModal({
  ...
  const { mutateAsync: createColumn, isPending, error } = useCreateColumn(boardId);
  const {
    register,
    handleSubmit,
    setValue,
    ...
    <div className="fixed inset-0 z-[var(--z-overlay)] flex items-center justify-center bg-black/75 backdrop-blur-[2px] animate-fade-in select-none">
      ...
      >
        ...
        <div className="flex items-center justify-between pb-3.5 border-b border-slate-200 dark:border-slate-800/60">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-foreground">Create Column</h3>
          <button
            onClick={onClose}
